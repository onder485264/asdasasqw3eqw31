...
client.on("message", async msg => {
  ...
  let a = await db.fetch(`kufur_${msg.guild.id}`)
    if (a == 'acik') {
      const küfür = [
        "yarak","s2k", "mk", "amk", "aq", "orospu", "oruspu", "oç", "sikerim", "yarrak", "piç", "amq", "sik", "amcık", "çocu", "sex", "seks", "amına", "orospu çocuğu", "sg", "siktir git","31","ananın amına yarak"
                  ]
...
client.on("ready", async () => {
  let sesliKanalID = client.channels.cache.get("967408093598220318");
  if (sesliKanalID)
    sesliKanalID.join()
